    ...
    for sents in pbar:
        ...
        if mi_net is not None:
            ...
            loss = train_step(net, sents, sents, 0.1, pad_idx,
                              optimizer, criterion, args.channel, args.lambda_mi, mi_net)
            pbar.set_description(
            ...
            loss = train_step(net, sents, sents, noise_std[0], pad_idx,
                              optimizer, criterion, args.channel, args.lambda_mi)
            pbar.set_description(
...
if __name__ == '__main__':
    ...
    if not os.path.isabs(args.vocab_file) and not args.vocab_file.startswith('data/'):
        args.vocab_file = os.path.join('data/europarl_processed/', os.path.basename(args.vocab_file))
    ...
    for epoch in range(args.epochs):
        ...
        if avg_acc < record_acc:
            ...
            deepsc_save_path = os.path.join(args.checkpoint_path, f'checkpoint_{str(epoch + 1).zfill(2)}.pth')
            torch.save(deepsc.state_dict(), deepsc_save_path)
            print(f"Saved DeepSC model to {deepsc_save_path}")

            # with open(args.checkpoint_path + '/checkpoint_{}.pth'.format(str(epoch + 1).zfill(2)), 'wb') as f:
            #     torch.save(deepsc.state_dict(), f)

            if mi_net is not None:
                if args.checkpoint_path.endswith('with_MI_AWGN'):
                    mi_net_save_path = os.path.join(args.checkpoint_path, f'mi_net_checkpoint_{str(epoch + 1).zfill(2)}.pth')
                    torch.save(mi_net.state_dict(), mi_net_save_path)
                    print(f"Saved MI Net model to {mi_net_save_path}")
